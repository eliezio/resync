...
def seed_config(catalog_path: str, existing_path: str | None = None,
    ...
    # Commented audit_override template — only for columns worth stamping (not VERSION/_IND).
    ov = []
    for c in audit:
        if c in ("UPDATE_ID", "INSERT_ID"):
            ov.append(f'#   {c}: "\'RESYNC\'"')
        elif c.endswith("_TMSTMP"):
            ov.append(f"#   {c}: SYSTIMESTAMP")
    override_block = ""
    if ov:
        override_block = (
            "# audit_override:            # optional: stamp audit columns with the re-sync instead of\n"
            "#                            # copying them (written on insert/update; matching unaffected)\n"
            + "\n".join(ov) + "\n"
        )
